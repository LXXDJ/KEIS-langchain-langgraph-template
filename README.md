...
각 카테고리에 **전용 프롬프트**를 사용하여 해당 카테고리의 데이터 특성에 맞는
요약을 생성합니다. 또한 카테고리별로 **전용 HTML 파서**가 구조화된 메타데이터를
추출하여 LLM 에 제공하므로, 단순 제목 나열이 아니라 의미 있는 요약이 가능합니다.
...
같은 검색어 "고용" 에 대해 카테고리별로 다른 관점의 요약이 생성됩니다:
...
- **카테고리별 파서/프롬프트의 정확도**: 각 카테고리의 전용 파서가 work24 의 HTML
  구조에 의존하므로, 마크업이 변경되면 메타데이터 추출이 깨질 수 있습니다. 특히
  채용의 D-day 뱃지, 훈련의 비용 구조, 정책의 지원대상 텍스트는 HTML 셀렉터에
  강하게 결합되어 있습니다.

## 설계 결정 배경 — 의도 기반 접근에서 카테고리별 요약으로

본 서비스의 초기 기획(SFR-04)은 **"사용자 질문 의도에 부합하는 핵심 정보를 선별하여
요약을 제공"** 하는 방향이었습니다. 구체적으로는:

1. 사용자의 검색어에서 **의도를 파악**하고
2. 9개 카테고리 중 **의도에 가장 부합하는 카테고리를 1순위로 선정**한 뒤
3. 해당 카테고리의 결과에서 **관련성 높은 콘텐츠를 선별**하여
4. **1순위 카테고리 중심의 단일 요약 카드**를 제공

이 접근에서 의도 분류는 LLM(GPT-4o mini)에게 검색어를 보내 9개 카테고리의
우선순위를 매기게 하는 방식으로 구현했었습니다.
...
work24 의 통합검색은 자연어 질문이 아니라 **키워드 검색**입니다. 사용자가 "고용",
"ai", "정책" 같은 단어만 입력하는 환경에서 LLM 이 의도를 정확히 추측하기
어렵습니다. "ai" 를 검색한 사람이 채용을 보려는 건지, 훈련을 보려는 건지,
정책을 보려는 건지 키워드만으로는 판단할 수 없습니다.

**2. 의도 추측이 틀리면 사용자에게 잘못된 정보를 제공합니다.**

LLM 이 1순위로 선정한 카테고리에 실제 결과가 0건인 경우 — 예를 들어 "ai"
검색에서 LLM 이 "훈련" 을 1순위로 골랐는데 work24 의 훈련 카테고리에 AI
관련 결과가 없는 경우 — 요약 카드가 비어버리는 문제가 발생했습니다.

**3. 명확한 검색어에서도 의도 분류가 불필요합니다.**

"클라우드 서비스 관련 정책" 같은 검색어는 사용자의 의도가 이미 명확합니다.
이 경우 LLM 의도 분류는 불필요한 비용(LLM 호출 1회)과 지연만 추가할 뿐
가치가 없습니다.

### 어떻게 풀어냈는가

의도를 추측하는 대신, **결과가 있는 모든 카테고리에 대해 각각 요약을 제공**하는
방식으로 전환했습니다. 이렇게 하면:

- **모호한 검색어** ("ai"): 결과가 있는 5~7개 카테고리 모두에서 각각의 관점으로
  요약을 제공 → 사용자가 직접 관심 있는 카테고리를 선택
- **명확한 검색어** ("클라우드 서비스 관련 정책"): 관련 카테고리에만 결과가 있으므로
  자연스럽게 해당 카테고리의 요약만 노출
- **의도 분류 LLM 호출 제거**: 비용 절감 + 응답 속도 개선

초기 기획의 "관련성 높은 콘텐츠 선별" 은 work24 의 자체 정확도순(RANK) 정렬에
위임하고, "사용자 의도 기반 요약" 은 카테고리별 전용 프롬프트 + 전용 파서가
각 카테고리의 특성에 맞는 핵심 정보를 추출·강조하는 방식으로 대체했습니다.
